...
import { handleJoinedVc } from "./handleJoinedVc.mjs";
import { handleUserTyping } from "./functions/handleUserTyping.mjs";
import { validateMemberId } from "./functions/validateMemberId.mjs";
import { escapeHtml } from "./functions/escapeHtml.mjs";
  ...
  if (fs.existsSync("./config.json")) {
    ...
    if (fs.existsSync("./config.example.json")) {
      ...
      try {
        ...
        consolas(" ", "Debug");
        consolas(
          "Successfully copied config.example.json to config.json".green,
          "Debug"
        );
      } catch (error) {
        consolas(
          "Coudlnt copy template file ".red + colors.red(error),
          "Debug"
        );
      }
    } else {
      consolas(
        "Neither the config.json file nor the config.example.json file were found."
          .red,
        "Debug"
      );
...
export let serverconfig = JSON.parse(
  fs.readFileSync("./config.json", { encoding: "utf-8" })
);

export const setServerconfig = (newConfig) => (serverconfig = newConfig);
...
if (serverconfig.serverinfo.ssl.enabled == 1) {
  ...
  consolas(
    "If accessed via the internet, SSL wont work and will cause problems".yellow
  );
  consolas(" ");
...
process.on("uncaughtException", function (err) {
  ...
  fs.writeFile(
    ...
    function (err) {
      ...
      consolas(
        "The log file ".cyan +
          colors.white("./logs/error_" + date + ".txt") +
          " was saved!".cyan,
        "Debug"
      );
    }
  ...
  fs.writeFile(
    "./config_backups/config_" + date + ".txt",
    JSON.stringify(serverconfig, false, 4),
    function (err) {
      if (err) {
        return console.log(err);
      }
      consolas(
        "The config file ".cyan +
          colors.white("./logs/error_" + date + ".txt") +
          " was saved!".cyan,
        "Debug"
      );
      ...
      if (args.length == 2) {
        ...
        if (isNaN(roleIdArg) == false) {
          try {
            ...
            consolas(
              colors.cyan(
                `Redeem key generated for role ${serverconfig.serverroles[roleIdArg].info.name}`
              )
            );
...
io.on("connection", function (socket) {
  ...
  socket.on("userConnected", function (member) {
      ...
      if (serverconfig.banlist[member.id] != null) {
        ...
        if (durationStamp == null || durationStamp <= 0) {
          ...
          if (currentDate > bannedDate) {
            ...
            consolas(
              colors.yellow(
                "Automatically unbanned user " + member.name + ` (${member.id})`
              )
            );
    ...
    if (member.id.length == 12 && isNaN(member.id) == false) {
      ...
      if (serverconfig.servermembers[member.id] == null) {
        ...
        if (
          serverconfig.serverroles["0"].members.includes(member.id) == false
        ) {
          ...
          sendMessageToUser(
            ...
            JSON.parse(
              `{
                        ...
                        "message": "Save the following token! #Without it you cant access your account on this server. Back it up now! ##Token: ${
                          serverconfig.servermembers[member.id].token
                        }",
                        "buttons": {
                            "0": {
                                "text": "Saved!",
        ...
        serverconfig.servermembers[member.id].status = escapeHtml(
          member.status
        );
        serverconfig.servermembers[member.id].aboutme = escapeHtml(
          member.aboutme
        );
        serverconfig.servermembers[member.id].icon = escapeHtml(member.icon);
        serverconfig.servermembers[member.id].banner = escapeHtml(
          member.banner
        );
        ...
        if (serverconfig.servermembers[member.id].isOnline == 0) {
          ...
          var lastOnline =
            serverconfig.servermembers[member.id].lastOnline / 1000;
  ...
  socket.on("audioStream", function (member) {
    if (
      ...
      socket.to(member.room).emit("audioData", {
        speakingMember: member.id,
        audioData: member.audioData,
      });
    }
  });
  ...
  socket.on("leftVC", function (member) {
    if (
      ...
      try {
        delete peopleInVC[member.room][member.id];
      } catch (error) {
        console.log("error deleting it");
        console.log(error);
      }
      socket.to(member.room).emit("userLeftVC", {
        id: member.id,
        name: serverconfig.servermembers[member.id].name,
        icon: serverconfig.servermembers[member.id].icon,
        room: member.room,
  ...
  socket.on("messageSend", function (member) {
    ...
    if (
      ...
      if (serverconfig.mutelist.hasOwnProperty(member.id)) {
        ...
        if (serverconfig.mutelist[member.id].reason.length > 0)
          var reasonText = `##Reason:#${
            serverconfig.mutelist[member.id].reason
          }`;
        ...
        if (serverconfig.mutelist[member.id].duration == -1) {
          ...
          // Mute ran out
          if (
            new Date(serverconfig.mutelist[member.id].duration).getTime() <
            new Date().getTime()
          ) {
            ...
            sendMessageToUser(
              ...
              JSON.parse(
                `{
                                    "title": "You're muted!",
                                    "message": "You cant send a message until ${new Date(
                                      serverconfig.mutelist[member.id].duration
      ...
      try {
        if (
          serverconfig.groups[member.group].channels.categories[member.category]
          ...
          if (
            serverconfig.groups[member.group].channels.categories[
              member.category
            ].channel[member.channel].type == "voice"
          ) {
            sendMessageToUser(
              socket.id,
              JSON.parse(
          ...
          member.message.replace(reg, function (emoji) {
            try {
              var emojiName = findEmojiByID(emoji.replaceAll(":", ""))
                .split("_")[2]
                .split(".")[0];
              member.message = member.message.replaceAll(
                emoji + ":",
                `<span><img title="${emojiName}" onerror="this.src='/img/error.png'" class="inline-text-emoji ${sendBigEmoji}" src="/emojis/${findEmojiByID(
                  emoji.replaceAll(":", "")
                )}"></span>`
              );
            } catch {}
          ...
          Object.keys(serverconfig.serverroles).forEach(function (role) {
            if (
              serverconfig.serverroles[role].members.includes(member.id) &&
        ...
        consolas(
          "Couldnt send message because room didnt exist".yellow,
          "Debug"
        );
  ...
  socket.on("getUserFromId", function (member, response) {
    if (validateMemberId(member.id, socket) == true) {
      response({
        type: "success",
        user: serverconfig.servermembers[member.target],
      });
      //io.to(usersocket[member.id]).emit("receiveChannelTree", getChannelTree(member));
    }
  });
  ...
  socket.on("getAllRoles", function (member, response) {
    if (validateMemberId(member.id, socket) == true) {
      ...
      Object.keys(sortedRoles)
        .reverse()
        .forEach(function (role) {
          //console.log(roles[role].info.name)

          if (
            sortedRoles[role].info.sortId < highestRole.info.sortId ||
            hasPermission(member.id, "administrator", member.group)
          ) {
            ...
            if (
              sortedRoles[role].info.sortId < executer.info.sortId &&
              role != 0 &&
              role != 1
            ) {
  ...
  socket.on("deleteEmoji", function (member, response) {
    ...
    if (validateMemberId(member.id, socket) == true) {
      if (hasPermission(member.id, "manageEmojis")) {
        ...
        response({
          type: "error",
          msg: "You dont have permissions to manage emojis",
        });
      }
    }
  });
      ...
      if (hasPermission(member.id, "manageBans")) {
        try {
          ...
          response({
            type: "success",
            msg: `The user ${
              serverconfig.servermembers[member.target].name
            } has been unbanned`,
          });
        } catch (e) {
      ...
      if (hasPermission(member.id, "manageMembers")) {
        try {
          ...
          if (!hasPermission(member.id, "administrator")) {
            if (
              serverconfig.serverroles[member.role].info.sortId >=
              highestUserRole.info.sortId
            ) {
              response({
                type: "error",
                msg: "You cant assign roles that are higher or equal then yours",
              });
              return;
            }
          }
        ...
        try {
          ...
          if (!hasPermission(member.id, "administrator")) {
            if (
              ...
              response({
                type: "error",
                msg: "You cant remove roles that are higher or equal then yours",
              });
              return;
            }
          }
          ...
          // Default Roles
          if (
            serverconfig.serverroles[member.role].info.id == 0 ||
            serverconfig.serverroles[member.role] == 1
          ) {
            return;
          }
  ...
  socket.on("saveRolePermissions", function (member, response) {
    if (
      ...
      if (hasPermission(member.id, "manageRoles")) {
        try {
          serverconfig.serverroles[member.role].permissions =
            member.permissions;
          saveConfig();
          ...
          response({
            type: "success",
            msg: "Role permissions have been updated",
          });
        } catch (e) {
    ...
    if (
      ...
      if (hasPermission(member.id, "manageChannels")) {
        try {
          ...
          serverconfig.groups[group].channels.categories[category].channel[
            memberChannel
          ].permissions[member.role] = member.permissions;
          saveConfig();

          io.emit("receiveChannelTree");
          response({
            type: "success",
            msg: "Channel permissions have been updated",
          });
        } catch (e) {
    ...
    if (
      ...
      if (hasPermission(member.id, "manageChannels")) {
        try {
          ...
          delete serverconfig.groups[group].channels.categories[category]
            .channel[memberChannel].permissions[member.role];
          saveConfig();

          response({
            type: "success",
            msg: "Role was successfully removed from the channel",
          });
        } catch (e) {
    ...
    if (
      ...
      if (hasPermission(member.id, "manageChannels")) {
        try {
          ...
          serverconfig.groups[group].channels.categories[category].channel[
            memberChannel
          ].permissions[member.role] = JSON.parse(
            `
                        {
                            "readMessages": 1,
                            "sendMessages": 1
  ...
  socket.on("addRoleToGroup", function (member, response) {
    if (
      ...
      if (hasPermission(member.id, "manageGroups")) {
        try {
          serverconfig.groups[member.group].permissions[member.role] =
            JSON.parse(
              `
                        {
                            "viewGroup": 1
          ...
          response({
            type: "success",
            msg: "Role was successfully added to the group",
          });
        } catch (e) {
    ...
    if (
      ...
      if (hasPermission(member.id, "manageGroups")) {
        try {
          ...
          response({
            type: "success",
            msg: "Role was successfully removed from the group",
          });
        } catch (e) {
  ...
  socket.on("updateRoleAppearance", function (member, response) {
    if (
      ...
      if (hasPermission(member.id, "manageRoles")) {
        try {
          serverconfig.serverroles[member.roleId].info.name =
            member.data.info.name;
          serverconfig.serverroles[member.roleId].info.color =
            member.data.info.color;
          serverconfig.serverroles[member.roleId].info.displaySeperate =
            member.data.info.displaySeperate;
    ...
    if (
      ...
      if (hasPermission(member.id, "manageRoles")) {
        try {
          ...
          response({
            type: "success",
            msg: "The role has been successfully created",
          });
        } catch (e) {
      ...
      if (hasPermission(member.id, "manageRoles")) {
        try {
          if (serverconfig.serverroles[member.roleId].info.deletable == 1) {
            ...
            response({
              type: "success",
              msg: "The role has been successfully deleted",
            });
          } else {
        ...
        for (var i = 0; i < member.permission.length; i++) {
          ...
          if (hasPermission(member.id, member.permission[i])) {
            ...
            response({
              type: "success",
              permission: "granted",
              user: serverconfig.servermembers[member.id],
            });
          } else {
  ...
  function banUser(member) {
    ...
    if (!serverconfig.ipblacklist.hasOwnProperty(ip)) {
      ...
      consolas(
        ` User ${
          serverconfig.servermembers[member.target].name
        } (IP ${ip}) was added to the blacklist because he was banned`.yellow
      );
        ...
        if (hasPermission(member.id, "muteUsers") == false) {
          ...
          if (member.time.length == 0) {
            ...
            sendMessageToUser(
              ...
              JSON.parse(
                `{
                                    "title": "You have been muted until ${new Date(
                                      mutedStatus
                                    ).toLocaleString()}",
                                    "message": "${reasonText}",
                                    "buttons": {
  ...
  socket.on("setRoom", function (member) {
    if (validateMemberId(member.id, socket) == true) {
      ...
      try {
        ...
        if (
          ...
          if (
            ...
            consolas(
              `User joined text room ${escapeHtml(member.room)}`.green,
              "Debug"
            );
          }
          // If its a voice channel
            ...
            consolas(
              `User joined VC room ${escapeHtml(member.room)}`.green,
              "Debug"
            );
          }
        }
      ...
      consolas(
        `Server Id ${serverconfig.servermembers[member.id].id}`.yellow,
        "Debug"
      );
      consolas(`User Id ${member.id}`.yellow, "Debug");
      consolas(
        `Server Token ${serverconfig.servermembers[member.id].token}`.yellow,
        "Debug"
      );
  ...
  socket.on("updateServerName", function (member, response) {
    if (
      ...
      if (hasPermission(member.id, "manageServerInfo")) {
        consolas(
          `Changing servername from ${
            serverconfig.serverinfo.name
          } to ${escapeHtml(member.value)}`,
          "Debug"
        );
        ...
        response({
          type: "error",
          msg: "You cant change the server name: Missing permissions",
        });
      }
    }
  });
  ...
  socket.on("updateServerDesc", function (member, response) {
    if (
      ...
      if (hasPermission(member.id, "manageServerInfo")) {
        consolas(
          `Changing server description from ${
            serverconfig.serverinfo.description
          } to ${escapeHtml(member.value)}`,
          "Debug"
        );
        ...
        response({
          type: "success",
          msg: "Server description was successfully changed",
        });
      } else {
        response({
          type: "error",
          msg: "You cant change the server description: Missing permissions",
        });
      }
    }
  });
  ...
  socket.on("getGroupList", function (member) {
    if (
      ...
      io.to(usersocket[member.id]).emit(
        "receiveGroupList",
        getGroupList(member)
      );
    }
  ...
  socket.on("getMemberList", function (member) {
    if (
      ...
      io.to(usersocket[member.id]).emit(
        "receiveMemberList",
        getMemberList(member, member.channel)
      );
    }
  });

  socket.on("isTyping", (member) =>
    handleUserTyping(socket, io, serverconfig, member)
  );
  ...
  socket.on("getGroupBanner", function (member) {
    if (
      ...
      io.to(usersocket[member.id]).emit(
        "receiveGroupBanner",
        serverconfig.groups[member.group].info.banner
      );
    }
  });
  ...
  socket.on("getChatlog", function (member) {
    if (
      ...
      if (
        checkUserChannelPermission(channel, member.id, "viewChannel") == true
      ) {
        io.to(usersocket[member.id]).emit(
          "receiveChatlog",
          getSavedChatMessage(member.group, member.category, member.channel)
        );
      }
    }
  });
  ...
  socket.on("createCategory", function (member) {
    if (
      ...
      try {
        ...
        serverconfig.groups[member.group].channels.categories[catId] =
          JSON.parse(
            `{
                        "info": {
                            "id": ${catId},
    ...
    if (
      ...
      try {
        ...
        if (member.value == null || member.value.length <= 0) {
          member.value =
            "https://t4.ftcdn.net/jpg/04/46/93/93/360_F_446939375_83iP0UYTg5F9vHl6icZwgrEBHXeXMVaU.jpg";
        }
  ...
  socket.on("updateGroupIcon", function (member) {
    if (
      ...
      try {
        if (member.value == null || member.value.length <= 0) {
          member.value =
            "https://wallpapers-clan.com/wp-content/uploads/2022/05/cute-pfp-25.jpg";
        }
  ...
  socket.on("createChannel", function (member) {
    if (
      ...
      try {
        consolas(
          `Trying to create channel in group ${member.group}, category ${member.category}`
            .yellow,
          "Debug"
        );
  ...
  socket.on("deleteChannel", function (member) {
    if (
      ...
      try {
        ...
        delete serverconfig.groups[group].channels.categories[category].channel[
          channelId
        ];
        saveConfig();
  ...
  socket.on("deleteCategory", function (member) {
    if (
      ...
      try {
        delete serverconfig.groups[member.group].channels.categories[
          member.category.replace("category-", "")
        ];
        saveConfig();
  ...
  socket.on("getCurrentChannel", function (member) {
    if (validateMemberId(member.id, socket) == true) {
      ...
      try {
        ...
        io.to(usersocket[member.id]).emit(
          "receiveCurrentChannel",
          serverconfig.groups[member.group]
        );
      }
    }
  });
    ...
    if (
      ...
      try {
        var message = JSON.parse(
          fs.readFileSync(
            `./chats/${member.group}/${member.category}/${member.channel}/${member.messageId}`
          )
        );

        if (
          message.id == member.id ||
          hasPermission(member.id, "manageMessages")
        ) {
          fs.unlinkSync(
            `./chats/${member.group}/${member.category}/${member.channel}/${member.messageId}`
          );
        ...
        consolas(
          `Couldnt delete file ./chats/${member.group}/${member.category}/${member.channel}/${member.messageId}`,
          "Debug"
        );
        consolas(error, "Debug");
      }
    ...
    if (
      ...
      try {
        io.to(usersocket[escapeHtml(member.id)]).emit("receiveMemberProfile", {
          code: await getMemberProfile(escapeHtml(member.target)),
          top: member.posY,
          left: member.posX,
        });
      } catch (e) {
      ...
      try {
        ...
        if (emojiList.length > 0) {
          response({
            type: "success",
            data: emojiList,
            msg: "Successfully received emojis",
          });
        } else {
    ...
    if (
      ...
      try {
        if (!hasPermission(member.id, "manageBans")) {
          ...
          response({
            type: "error",
            msg: "You dont have permissions to manage Bans",
          });
          return;
        }
        ...
        response({
          type: "success",
          data: serverconfig.banlist,
          msg: "Successfully received banlist",
        });
      } catch (e) {
        consolas("Couldnt get emojis".red, "Debug");
        consolas(colors.red(e), "Debug");
  ...
  socket.on("updateEmoji", async function (member, response) {
    ...
    if (
      ...
      try {
        ...
        var newEmoji = `emoji_${member.emojiId}_${member.emojiName}.${oldEmoji
          .split(".")
          .pop()}`;
        ...
        fs.rename(
          "./public/emojis/" + oldEmoji,
          `./public/emojis/` + newEmoji,
          function (err) {
            if (err) {
              response({
                type: "error",
                error: err,
                msg: "Couldnt update emoji",
              });
            } else {
  ...
  socket.on("fileUpload", function (member, response) {
    ...
    if (!hasPermission(member.id, "uploadFiles")) {
      ...
      response({
        type: "error",
        msg: "You dont have permissions to upload files",
      });
      return;
    }
      ...
      if (serverconfig.serverinfo.uploadFileTypes.includes(filetype.mime)) {
        ...
        if (
          ...
          if (serverconfig.serverinfo.useCloudflareImageCDN == 1) {
            ...
            fetch(
              `https://api.cloudflare.com/client/v4/accounts/${cloudflareId}/images/v1`,
              {
                method: "POST",
                headers: {
              ...
              if (result.status == 200) {
                consolas(
                  "Uploaded url: " +
                    `https://imagedelivery.net/${cloudflareHash}/${cloudname}/public`,
                  "Debug"
                );
                response({
                  type: "success",
            ...
            getSize(localUploadPath, (err, size) => {
              ...
              if (userUpload == null) {
                if (hasPermission(member.id, "bypassUploadLimit")) {
                  ...
                  response({
                    type: "error",
                    msg: "You are not allowed to upload files",
                  });
                  return;
                }
              }
              ...
              if (fileSizeMB > userUpload) {
                ...
                consolas(
                  `Max User Upload Size: ${userRole.permissions.maxUpload}`,
                  "Debug"
                );
              ...
              var currentFolderSize = Math.round(
                (size / 1024 / 1024).toFixed(2)
              );

              if (currentFolderSize < maxFolderSize) {
                consolas(
                  `Member ${member.id} is uploading the file ${member.filename}`,
                  "Debug"
                );
                var error;
                ...
                if (error == null) {
                  ...
                  response({
                    type: "success",
                    msg:
                      localUploadPath.replace("./public", "") + "/" + filename,
                  });
                ...
                consolas(
                  `Cannot upload file! Max Upload Size Limit ${maxFolderSize}MB reached!`
                    .yellow
                );
                response({
                  type: "error",
                  msg: "Server's max. upload limit reached",
                });
              }
            });
          }
        ...
        response({
          type: "error",
          msg: "This file type is not allowed on the server",
        });
        consolas(
          "File Type for upload not allowed. File Type: " + filetype.mime
        );
      }
    });
  });
  ...
  socket.on("getServerInfo", function (member, response) {
    ...
    if (
      ...
      consolas(
        "ID or Token was invalid while requesting server information",
        "Debug"
      );
  ...
  socket.on("getGroupInfo", function (member, response) {
    ...
    if (
      ...
      if (hasPermission(member.id, "manageGroups")) {
        ...
        response({
          type: "success",
          msg: "Successfully resolved group",
          data: groupObj,
        });
      } else {
        response({
          type: "error",
          msg: "You dont have the permissions to manage groups",
        });
      }
    } else {
      consolas(
  ...
  socket.on("getChannelInfo", function (member, response) {
    ...
    if (
      ...
      if (hasPermission(member.id, "manageChannels")) {
        var channelObj = resolveChannelById(
          member.channel.replace("channel-", "")
        );
        response({
          type: "success",
          msg: "Successfully resolved channel",
          data: channelObj,
        });
      } else {
        response({
          type: "error",
          msg: "You dont have the permissions to manage channels",
        });
      }
    } else {
      consolas(
  ...
  socket.on("getServerRoles", function (member, response) {
    ...
    if (
      ...
      if (hasPermission(member.id, "manageRoles")) {
        ...
        response({
          type: "error",
          msg: "You dont have the permissions to manage server roles",
        });
      }
    } else {
      consolas(
  ...
  socket.on("getGroupChannels", function (member, response) {
    ...
    if (
      ...
      if (
        hasPermission(member.id, "manageChannels") ||
        ...
        response({
          type: "error",
          msg: "You dont have the permissions to manage group channels",
        });
      }
    } else {
      consolas(
    ...
    if (
      ...
      if (
        ...
        try {
          ...
          response({
            type: "success",
            msg: "Changes were successfully applied",
          });

          // Update Channel Hierarchy for everyone
          io.to(usersocket[member.id]).emit(
            "receiveChannelTree",
            getChannelTree(member)
          );
          io.emit("receiveChannelTree", getChannelTree(member));
    ...
    if (
      ...
      if (hasPermission(member.id, "manageUploads")) {
        try {
          ...
          serverconfig.serverinfo.cfAccountToken =
            member.cloudflareAccountToken;
          serverconfig.serverinfo.cfHash = member.cloudflareHash;
          saveConfig();

          response({
            type: "success",
            msg: "Settings saved successfully, please try to upload a profile picture to see if it works.",
          });
        } catch (error) {
          response({
            type: "error",
            msg: "Server couldnt save settings: " + error,
          });
        }
      } else {
        response({
          type: "error",
          msg: "You dont have the permissions to manage the upload settings",
        });
      }
    }
  });
    ...
    if (
      ...
      if (hasPermission(member.id, "manageRateSettings")) {
        try {
          ...
          response({
            type: "error",
            msg: "Server couldnt save rate settings: " + error,
          });
        }
      } else {
        response({
          type: "error",
          msg: "You dont have the permissions to manage the rate settings",
        });
      }
    }
  });
...
function hasPermission(id, permission, searchGroup = null) {
  ...
  if (searchGroup != null) {
    ...
    Object.keys(serverconfig.groups[searchGroup].permissions).forEach(function (
      permrole
    ) {
      ...
      if (userroles.includes(permrole)) {
        ...
        Object.keys(
          serverconfig.groups[searchGroup].permissions[permrole]
        ).forEach(function (perm) {
          if (
            permission == perm &&
            serverconfig.groups[searchGroup].permissions[permrole][perm] == 1
          ) {
            ...
            permission == perm &&
            serverconfig.groups[searchGroup].permissions[permrole][perm] == 0
          ) {
            foundPermission = false;
          }
        });
  ...
  Object.keys(serverconfig.groups).forEach(function (group) {
    // For each Group Permission Role
    Object.keys(serverconfig.groups[group].permissions).forEach(function (
      permrole
    ) {
      // If the user role includes the group role
      if (userroles.includes(permrole)) {
        // For each permission of the group role
        Object.keys(serverconfig.groups[group].permissions[permrole]).forEach(
          function (perm) {
            if (
              permission == perm &&
              serverconfig.groups[group].permissions[permrole][perm] == 1
            ) {
              foundPermission = true;
            } else if (
              permission == perm &&
              serverconfig.groups[group].permissions[permrole][perm] == 0
            ) {
              foundPermission = false;
            }
          }
    ...
    userroles.forEach((userrole) => {
      if (serverconfig.groups[group].permissions[userrole] != null) {
        if (
          serverconfig.groups[group].permissions[userrole][permission] == 1 ||
  ...
  userRoles.forEach((role) => {
    ...
    if (group != null && category != null && channel != null) {
      ...
      if (
        serverconfig.groups[group].channels.categories[category].channel[
          channel
        ].permissions[role].hasOwnProperty(perm)
      ) {
...
function resolveGroupByChannelId(id) {
  ...
  var found = null;
  // Foreach Group
  Object.keys(serverconfig.groups)
    .reverse()
    .forEach(function (group) {
      //console.log(group);

      // For each Category
      Object.keys(serverconfig.groups[group].channels.categories)
        .reverse()
...
function resolveRolesByUserId(id) {
  var userRoles = [];
  // Foreach Group
  Object.keys(serverconfig.serverroles)
    .reverse()
    .forEach(function (roles) {
          ...
          Object.keys(
            ...
            .forEach(function (channelId) {
              ...
              if (channelId == id) {
                ...
                found =
                  serverconfig.groups[group].channels.categories[category]
                    .channel[id];
                return serverconfig.groups[group].channels.categories[category]
                  .channel[id];
              }
            });
        });
...
async function getUserBadges(id) {
  return new Promise((resolve, reject) => {
    var badgeUrl =
      "https://raw.githubusercontent.com/hackthedev/dcts-shipping/main/badges/" +
      serverconfig.servermembers[id].id;
    ...
    var profile = `
        ...
        <div id="profile_content">
            ...
            <div class="profile_aboutme">            
                ${memberAboutme}<br><br>
                <code class="joined">Joined ${new Date(
                  memberJoined
                ).toLocaleString("narrow")}</code><br>
                <code class="joined">Last Online ${new Date(
                  memberLastOnline
                ).toLocaleString("narrow")}</code>
                ${mutedBadge}
            </div>
            <hr>
...
function getSavedChatMessage(group, category, channel) {
  ...
  fs.readdirSync(`./chats/${group}/${category}/${channel}/`).forEach((file) => {
    var message = JSON.parse(
      fs.readFileSync(`./chats/${group}/${category}/${channel}/${file}`)
    );

    if (
      message.message.includes("<br>") &&
      message.message.split("<br>").length - 1 <= 1
    ) {
...
function saveChatMessage(message) {
  ...
  if (!fs.existsSync(`./chats/${group}/${category}/${channel}/`)) {
    fs.mkdirSync(`./chats/${group}/${category}/${channel}/`, {
      recursive: true,
    });
  }
  ...
  fs.writeFile(
    `./chats/${group}/${category}/${channel}/${message.messageId}`,
  ...
  sortedGroups.forEach((group) => {
    ...
    if (
      ...
      userroles.forEach((role) => {
        try {
          if (
            group.permissions[role].viewGroup == 1 &&
            addedGroups.includes(group.info.id) == false
          ) {
            addedGroups.push(group.info.id);
...
function getChannelTree(member) {
  ...
  sortedCats.forEach((cat) => {
    ...
    const chanCollection =
      serverconfig.groups[group].channels.categories[cat.info.id].channel;
    ...
    sortedChans.forEach((chan) => {
      ...
      Object.keys(roles).forEach(function (role) {
        if (roles[role].members.includes(member.id)) {
          if (
            checkUserChannelPermission(chan.id, member.id, "viewChannel") ||
            hasPermission(member.id, "manageChannels", member.group)
          ) {
...
function saveConfig() {
  fs.writeFileSync(
    ...
    function (err) {
      ...
      consolas(
        "The config file ".cyan +
          colors.cyan("./logs/error_" + date + ".txt") +
          " was saved!".cyan
      );
    }
  );
      ...
      if (
        ...
        isImgUrl(url).then((result) => {
          if (result == true) {
            ...
            io.in(roomid).emit("createMessageEmbed", {
              messageId: messageid,
              code: code,
            });

            resolve(
              `<a href="${url}" target="_blank">${url}</a><div class="iframe-container" id="${messageid}"><img class="image-embed" src="${url}"></div>`
            );
            ...
            io.in(roomid).emit("createMessageLink", {
              messageId: messageid,
              code: code,
            });
